...
| Table | Retention | Why |
|---|---|---|
| `raw_requests` | 30 days | untouched payloads, so a rule change can be replayed without re-fetching |
| `clean_requests` | 60 days | the serving window |
| `validation_failures` | 30 days | a failure whose record is gone cannot be investigated |
